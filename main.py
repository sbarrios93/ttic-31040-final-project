...
@click.option("save_result", "--save", "-s", is_flag=True, default=False, help="Save result to file")
@click.option(
    "save_path",
    "--save-path",
    "-p",
...
def run_pipeline(
    ...
    save_result: bool,
    save_path: typing.Union[str, Path, None, int],
    save_format: str,
    k_param: int,
    ...
    if save_result and (save_path is not None):
        ...
        if save_path.is_file():
            ...
            if not save_path.parent.exists():
                click.UsageError(f"{save_path.parent} does not exist, create it first and run again")
        ...
        if save_path.is_dir():
            if not save_path.exists():
                click.UsageError(f"{save_path} does not exist, create it first and run again")
    ...
    with click.progressbar(queue, label="Processing") as bar:
        for path in bar:
            ...
            if save_result:
                if save_path.is_file():
                    ...
                    fig_name_w_extension = save_path.name
                else:
                    fig_name_w_extension = (
                        path.stem + "." + save_format
                    )  # reuse filename from loaded path, append extension set in command line
                    fig_dir = save_path
            else:
                fig_dir = None
                fig_name_w_extension = None

            if not dont_display_result:
                show = True
            else:
                show = False

            drawer = Drawer(bbox_corners=bbox, image=detector._lookup_image)
